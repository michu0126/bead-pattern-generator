...
APP_VERSION = "0.4.0"
VERSION_CHANGES = [
    "新增受管理密码保护的 OpenAI 兼容 API 设置页面",
    "支持自定义 API URL、API Key、图像模型与质量，并可测试连接",
    "API 设置持久化到 /data 数据卷，更新或重建容器后不会丢失",
    "新增云端智能抠图，透明遮罩会重新套回原图以保留原始颜色",
    "图纸支持点击单个格子修改 MARD 色号、设为空白及逐步撤销",
]
