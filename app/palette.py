"""MARD standard 221-colour digital HEX reference palette.

The A-H and M codes are the MARD identifiers shown on patterns and retail stock.
HEX values are stored losslessly as 24-bit sRGB and are used as exact matching
keys before any nearest-colour calculation.
"""
...
def _hex_to_rgb(value: str) -> tuple[int, int, int]:
    value = value.lstrip("#")
    if len(value) != 6:
        raise ValueError(f"无效的 MARD HEX：{value!r}")
...
BEAD_PALETTE = [
    ...
    for code, value in MARD_2_6_HEX.items()
]

assert len(BEAD_PALETTE) == 221
assert len(MARD_2_6_HEX) == len(set(MARD_2_6_HEX))
assert all(value.startswith("#") and len(value) == 7 for value in MARD_2_6_HEX.values())
